...
export function lookProbe(engine: AbstractEngine, current: () => { scene: Scene; lighting: DungeonLighting } | null) {
  ...
  return {
    ...
    gpuTime(frames = 240, timeoutMs = 20000): Promise<FrameCost> {
      ...
      // The frame meter keeps the engine's GPU timer running for the life of the page; asking again is harmless.
      const gpu = engine.getCaps().timerQuery ? (engine.captureGPUFrameTime(true), engine.getGPUFrameTimeCounter()) : null;
      return new Promise((resolve, reject) => {
        let seen = 0, settled = 0, frameSum = 0, counted = 0, total = 0, samples = 0;
        const finish = (error?: Error) => {
          ...
          const gained = gpu ? gpu.count - samples : 0;
        ...
        const observer = scene.onAfterRenderObservable.add(() => {
          ...
          if (settled < 30) {
            ...
            if (settled === 30 && gpu) { total = gpu.total; samples = gpu.count; }
            return;
          }
