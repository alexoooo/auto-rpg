...
export function mulberry32(seed: number): () => number {
  ...
  // A fork of the world (`src/forkable.ts`) carries the stream's position, and a fork that wants a
  // different future from the same moment starts the stream again from a new seed.
  return Object.assign(next, {
    captureState: (): Record<string, unknown> => ({ state }),
    restoreState: (saved: Record<string, unknown>): void => { ({ state } = saved as never); },
    reseed: (to: number): void => { state = to >>> 0; },
  });
}
