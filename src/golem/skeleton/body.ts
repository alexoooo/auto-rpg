 ...
 *
 * **How hard it is to knock over is its geometry's** (physical contact session 08): a light pelvis
 * on thin legs, a high centre of mass over two narrow soles, read off the live body like every other
 * (`src/tipping.ts`). It used to carry a brace of 2.0 against stone's 1.5, chosen by a table of
 * skeleton mirrors under the frozen lines, and that table went with the brace; it is in the history
 * of this file.
 *
 ...
 * stone's knockdown. That is why a skeleton's rise was once made to hold through hits
 * (`riseHoldsThroughHits`, removed by physical contact session 02). The table is every body's now
