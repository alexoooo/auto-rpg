...
/** What a family may give its own wrist chain beyond the two tables. */
    ...
    build(
      ...
      const parts: readonly GolemPart[] = Object.freeze([
        ...
        Object.freeze({
          ...
          fatal: false,
          ...(options.armour === undefined ? {} : { armour: options.armour }),
        }),
        Object.freeze({
          id: link.name,
          part: link,
      ...
      return Object.freeze({
        ...
        castToCarried(inertiaKgM2: number): void {
          ...
          for (const part of [ring, link]) {
            ...
            if (!own) continue;
            const cast = Math.max(own.x, own.y, own.z, want, R.jointInertiaFloor);
...
/** The stone wrist: `wristChainFrom` over the stone arm's own tables. */
