...
- **The repository's only actuator sweep harness varied two constants, neither of them one
  ...
  bracketing every candidate between two production controls asserted byte-for-byte equal,
  and it had no candidate type for a hip rate, a twist budget or an elbow stop. It drove
  the articulated model and was deleted with it; its shape survives in the
  [calibration record](smart-ai-actuator-calibration.md#method) and nowhere else.
...
|---|---|---|
| `STANCE_TWIST_LIMIT_RAW` | a judgement about play, with both failure modes named | [`the_twist_limit_is_bounded_from_both_sides`](../../crates/sim/src/world/mod.rs#L2678) |
| `STANCE_HIP_MOVING_SPEED_RAW` | derived: the torso's own rate, unchanged | [`a_moving_body_turns_its_hips_faster_than_a_standing_one`](../../crates/sim/src/world/mod.rs#L2575) |
| `STANCE_HIP_STANDING_SPEED_RAW` | derived: half the moving rate, and the ratio is the mechanic | [`the_standing_hip_rate_is_bounded_from_both_sides`](../../crates/sim/src/world/mod.rs#L2688) |
| `STANCE_HIP_ACCEL_RAW` | derived: the torso's acceleration, unchanged | nothing bounds it; it is an equality by definition |
| `STANCE_STEP_TICKS` | arithmetic on the two rates, plus one upper judgement | [`a_forced_step_outlasts_the_turn_it_exists_to_make`](../../crates/sim/src/world/mod.rs#L2729) |
| `STANCE_STEP_MOVE_AUTHORITY_RAW` | a judgement: not zero, not one | [`the_step_authority_is_bounded_from_both_sides`](../../crates/sim/src/world/mod.rs#L2696) |
| `PELVIS_HEIGHT_RAW` | anatomy: the pelvis is halfway up a standing body | [`pelvis_height_falls_with_speed_and_with_twist_and_is_never_commanded`](../../crates/sim/src/world/mod.rs#L2612), on the base value only |
...
| `ELBOW_PLANE_MAX_SPEED_RAW` | derived from a **measured** constant | [`the_elbow_plane_rate_is_bounded_from_both_sides`](../../crates/sim/src/world/mod.rs#L2716) |
...
is not modelled — `bill_fatigue_for_grip` charges the hand's travel and the bearing's sweep, both of
...
[`a_hand_can_never_be_further_from_its_shoulder_than_the_arm_is_long`](../../crates/sim/src/world/mod.rs#L2511)
...
- **It needs a harness that does not exist.** The command named on the constants,
  ...
  `lab embodied`'s, and the candidate machinery to copy is the one
  `strike-corpus --calibrate-actuator` had before that command was deleted with the
  articulated model: a candidate type, a ladder, and every candidate bracketed between two
  controls asserted equal. That shape is now read out of the
  [calibration record](smart-ai-actuator-calibration.md#method) rather than out of a file,
  which makes the debt larger and not smaller.
