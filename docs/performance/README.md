...
| smart articulated combat | controlled Robust Strike is visibly verified; Smart128 stopped structurally, Smart129 found nine controlled-arm count mismatches, and Smart130 localized the first to tick 46 without establishing causality | [tactical-policy outcome](smart-ai-tactical-policy.md) and [matched Tactical/control evidence](smart-ai-matched-tactical.md#frozen-ordinal-31-arm-provenance) | diagnose the frozen tick-46 segment/body transcript behind reference `budget` and held's absent rejected-pair diagnostic; generalized competence remains `21/100` strict and `55/100` outcome-only, with no training, default promotion or `ARTICULATED_HASH` |
