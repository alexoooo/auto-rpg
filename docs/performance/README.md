...
| smart articulated combat | controlled Robust Strike is visibly verified; Smart128 stopped structurally, Smart129 found nine controlled-arm count mismatches, Smart130 localized the first to tick 46, and Smart131 found the first bounded path difference at the pair AABB control | [tactical-policy outcome](smart-ai-tactical-policy.md) and [matched Tactical/control evidence](smart-ai-matched-tactical.md#frozen-ordinal-31-tick-46-pair-transcript) | diagnose only the frozen pair's swept-AABB operands and axis early exit behind reference `2` regions versus held `0`; generalized competence remains `21/100` strict and `55/100` outcome-only, with no mechanics change, training, default promotion or `ARTICULATED_HASH` |
