...
| `ARTICULATED_COMMAND_HASH` | **moves.** This row used to be grouped with the one above and the grouping was wrong twice over. It is `world.state_digest().value` of an unstepped fixture, so it folds `legacy_core_hash` like every state-digest pin; session 01 moved it. It moves *here* for a second and independent reason the row did not have -- but **not the reason this row gave until 2026-08-19**, which was that its fixture is `init_articulated_test`. That was wrong: `init_articulated_test` was deliberately **not** reseated, on a measurement -- the boundary clinch's world-frame byte table gives zero contacts in four hundred ticks on an embodied duel, so reseating it would have silently deleted a fixture and six tests. A new `init_embodied_test` was added beside it and *that* became the pin's fixture, which is what actually moved the number. The correction is worth keeping because the two accounts predict the same move for different causes, and only one of them tells the next reader where to look |
