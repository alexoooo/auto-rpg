...
**Status:** ready. Independent of session 01. Blocks 03, 04 and 05.
...
/// the same thing.
///
/// **Measured from the yaw the command itself requests, and not from `obs.body_yaw`.**
/// That is the trap this whole function exists around. The planner asks for a turn
/// and a cut on the same tick; if the cut's bearing were taken relative to where the
/// body *is* rather than where it is being *sent*, the two would describe one
/// intention half a turn apart, and the blade would arrive where the torso used to
/// face. `crates/web`'s `drive_hero` already makes this choice for the human hand
/// and says so; this is the same sentence for the AI hand.
fn into_torso_frame(world: ArticulatedCommandV1) -> EmbodiedCommandV1 {
    let facing = world.body_yaw;
    let mut out = world;
...
impl EmbodiedPolicy for TacticalEmbodiedPolicy {
    fn decide(&mut self, obs: &ArticulatedObservation) -> EmbodiedCommandV1 {
        into_torso_frame(self.planner.decide(obs))
    }
...
The second is the one that carries the session. `the_same_situation_at_two_yaws_produces_one_command`
