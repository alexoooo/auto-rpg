...
| [Embodied combat](embodied-00-overview.md) | active; the two refactor sessions and the third `CombatModel` have landed with no existing pin moved | two refactor sessions gate everything; then a third `CombatModel` carrying terrain elevation, stance, and a two-link arm | sessions 01--08 move nothing except `ARTICULATED_STREAM_DIGEST` by extension in 06 and 07; session 09 owns `FEATURE_LAYOUT_VERSION` |
