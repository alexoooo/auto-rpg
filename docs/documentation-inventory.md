...
| [`DESIGN.md#the-agent-boundary`](../DESIGN.md#the-agent-boundary) | The agent boundary | architecture | current | v2-04 | `docs/architecture/policy.md` | Stable compatibility entry; architecture owns the flow and command reference owns exact feature layouts. Root orientation prose now links those authorities. |
...
| [`AGENTS.md#golden-hashes-decide-before-you-edit-not-after`](../AGENTS.md#golden-hashes-decide-before-you-edit-not-after) | Golden hashes: decide before you edit, not after | reference | current | v2-05 | none | Operational change gate links the canonical registry for exact hash names, pin locations, and re-record paths. The `ROOM_HASH`/`Order::Goto` trap that used to live here was deleted on 2026-08-18 with the pin and the export it named: `ROOM_HASH` is in the retired table and `set_goto` is gone from `crates/web`, so the paragraph had become a reader's instruction to protect a byte stream that no longer exists. |
...
| [`AGENTS.md#the-frame-abi-is-a-handshake-across-five-files`](../AGENTS.md#the-frame-abi-is-a-handshake-across-five-files) | The frame ABI is a handshake across five files | reference | current | v2-05 | none | Required workflow remains here; the obligation list, the versions, the identity rules and the count now live in `frame-abi.md`, which also carries why the number has been four, six and five. The heading has been wrong twice, so count from the reference rather than from it. |
...
| [`AGENTS.md#gotchas-that-have-already-cost-time`](../AGENTS.md#gotchas-that-have-already-cost-time) | Gotchas that have already cost time | evidence | current | keep root | none | Operational traps compressed to their rule. The browser-tab and native-benchmark methods, and the CPU-pinning advice that went stale when the host changed, moved to `docs/performance/README.md` on 2026-08-18; the `script_digest` defect moved to `docs/architecture/policy.md` and the ordered-movement fact to `docs/reference/commands.md`. |
| [`AGENTS.md#before-you-call-it-done`](../AGENTS.md#before-you-call-it-done) | Before you call it done | reference | current | keep root | `docs/README.md` | Contributor completion gate. `check_docs.js` asserts this section names `cargo test`, `wasm_check.js`, a hash step, the ABI mirrors, documentation impact and `check_docs.js` itself, so a rewrite that defers to "the gate above" fails the checker. |
