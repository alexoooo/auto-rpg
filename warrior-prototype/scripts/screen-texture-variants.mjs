...
if (variants.length === 0) throw new Error("provide one or more texture variant names");
...
for (const variant of variants) {
  const review = resolve(root, ".review/screen/textures", variant);
  mkdirSync(review, { recursive: true });
  const result = spawnSync(executable, ["--background", "--factory-startup", "--python",
    resolve(root, "asset-src/v3/screen_texture_variants.py"), "--",
