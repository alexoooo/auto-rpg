...
if (variants.length === 0) throw new Error("provide one or more pauldron variant names");
...
for (const variant of variants) {
  const review = resolve(root, ".review/screen/pauldrons", variant);
  mkdirSync(review, { recursive: true });
  const result = spawnSync(executable, ["--background", "--factory-startup", "--python",
    resolve(root, "asset-src/v3/screen_pauldron_variants.py"), "--",
