...
if (variants.length === 0) throw new Error("provide one or more body variant names");
...
for (const variant of variants) {
  const review = resolve(root, ".review/screen/body", variant);
  mkdirSync(review, { recursive: true });
  const result = spawnSync(executable, ["--background", "--factory-startup", "--python",
    resolve(root, "asset-src/v3/screen_body_variants.py"), "--",
