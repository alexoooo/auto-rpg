...
export interface EdgeGene { innovation: number; from: number; to: number; weight: number; enabled: boolean }
...
export function hasCycle(genome: Pick<Genome, "nodes" | "edges">): boolean {
  ...
  for (const edge of genome.edges) if (edge.enabled) {
...
export function addNodeMutation(genome: Genome, rng: SeededRng, innovations: InnovationTracker): boolean {
  const enabled = genome.edges.filter((candidate) => candidate.enabled); if (!enabled.length) return false; const edge = rng.choose(enabled);
...
export function crossover(fitter: Genome, other: Genome, rng: SeededRng, id: number): Genome {
  ...
  const edges = fitter.edges.map((edge) => ({ ...(otherEdges.has(edge.innovation) && rng.chance(0.5) ? otherEdges.get(edge.innovation) : edge) as EdgeGene }));
  const nodeIds = new Set(edges.flatMap((edge) => [edge.from, edge.to]));
  const otherNodes = new Map(other.nodes.map((node) => [node.id, node]));
  const nodes = fitter.nodes.filter((node) => node.kind === "input" || node.kind === "bias" || node.kind === "output" || nodeIds.has(node.id))
    .map((node) => ({ ...(otherNodes.has(node.id) && rng.chance(0.5) ? otherNodes.get(node.id) : node) as NodeGene }));
  return { id, nodes, edges, fitness: 0, adjustedFitness: 0, novelty: 0 };
}
...
export function breedGeneration(population: Genome[], seed: number, elite: number, innovations: InnovationTracker): Genome[] {
  const rng = new SeededRng(seed); const groups = speciate(population); const next: Genome[] = [];
