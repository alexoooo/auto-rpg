...
byte-identical. The set's baseline is re-taken so the rule's effect is one table.
...
- **No other rule moves.** Speed floors, the bite table, `hitCooldown`, armour, severing and the
  vitality weights are as they were. If the entry finds a bout now runs too long, the number to
  move is the 60 s cap, and it is moved with the owner, not silently.
