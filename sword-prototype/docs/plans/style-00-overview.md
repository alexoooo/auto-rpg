...
> **2026-09-08 status: Sessions 00 to 13 implemented; their human gates are open, and
> nothing is accepted.**
> Sixteen files, one per landable session. Sessions 14 and 15 are unimplemented and every gate,
> Sessions 00's to 13's included, is open. Every session ends at a human gate that
...
| 13 | nothing; the bar is that the policy beats a uniform command by Session 11's effect size and its reward curve rises | open, and not asked: **the bar is missed and the curve rises**. `golem-policy` is a 71 -> 256 -> 256 -> 12 head fitted by PPO in mirrored self-play against a reward that pays at every ask, sixty iterations over 1,920 bouts and 3,348,480 asks. Against the uniform-command baseline on 1,030 bouts a contender it is **+0.0871 +- 0.0247 of bar, d +0.216**, where the plan asked for d 0.235 and the standard error on d is 0.044 -- 0.4 standard errors short, with the interval containing the target. Rated at the same thousand bouts the untrained head is worth +0.0030 +- 0.0209 over uniform -- nothing -- so the whole **+0.0841 +- 0.0324 between the two ends of the curve, 5.1 standard errors, was put there by the fit**, and a weighted fit of all thirteen rating points agrees at +1.29e-3 +- 2.53e-4 an iteration, t 5.10. The policy ends level with `golem-driver` at -0.0145 +- 0.0206 (d -0.043) from -0.0987 +- 0.0213 (d -0.283) at iteration zero. Two things rewrote the trainer: the mirrored self-play return is zero by construction, so the curve has to be a periodic rating; and Adam's step is +-`rate` a weight whatever the gradient, so at 3e-4 with a 512-sample minibatch the trust region stopped every fit after **one** minibatch, spending 512 of 57,000 samples. The fitted mind throws the fewest strokes, gets the least speed into them and deals the least damage of the three contenders -- and loses least, draws most, stays inside the inner radius 28.7 % of the time and wins with **half its bar left** (0.512 against 0.362 and 0.367). Both halves of the ledger say why: random commands deal 23.98 a bout and take 31.03, the fit deals 20.54 and takes 20.46, so **the whole advantage over the baseline is on the taking side**. It is worst of the three on both penalised columns, which the entry reports rather than explains away. See the Session 13 entry of `../measurements.md` |
