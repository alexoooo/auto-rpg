...
- `Combatant` in `src/units.ts#L44` is the arena boundary, but its public `mind`, `view` and
...
- `UnitDefinition` in `src/units.ts#L94` remains the registry entry. A construct definition points
...
| [04](construct-forge-04-bronze-warden.md) | one fixed Bronze Warden selectable beside existing units | 02, 03 |
| [05](construct-forge-05-action-runtime.md) | groups, closed-loop actions, claims and concurrent scheduler | 04 |
| [06](construct-forge-06-quadruped-locomotion.md) | four generic limbs configured as locomotion | 05 |
| [07](construct-forge-07-articulated-mounts.md) | one two-axis mount drives either sword or auto-crossbow | 05 |
...
| [16](construct-forge-16-integration-and-playtest.md) | player verdict, promotion decision and durable close-out | 15 |
...
- `CONSTRUCT_OBSERVATION_VERSION = 1` -- variable graph features and mirror rules.
- `CONSTRUCT_POLICY_VERSION = 1` -- graph encoder, candidate scorer and checkpoint codec.

Each version owns a canonical JSON spelling and integrity checksum. The browser-safe FNV-1a
integrity implementation in `src/learning/artifact.ts#L110` may be reused; SHA-256 remains the
