# Session 17 -- execute recurrent PPO

## Outcome

Run both equal-budget PPO initialization arms for each seed against the frozen shipped and
DAgger league, then retain the validation-selected artifact. PPO already trains the
movement/action/value heads and all GRU gates with deterministic truncated BPTT.
...
- The report prints every reward component, optimizer/clipping/entropy diagnostics,
  recurrent gradient norms, validation macro and worst-cell values.
