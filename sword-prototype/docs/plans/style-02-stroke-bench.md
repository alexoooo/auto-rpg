...
committed stroke shapes chosen from it for Session 03; and the plate's arrival time over a parry
distance, which decides whether the guardian's parry is an intercept or a wall.
...
- **The probe reads the mark-plane crossing.** The step where the tip's swing angle about the
  ...
  `--sweep stroke`.
- **The grid** on the wrist blade, then the club and the fist: chamber swing {0.05, 0.4, 0.8,
  ...
  not on the bench, which has no carrier; they are swept in Session 03 through the contact-speed
  column.
- **The parry sequence** for the plate: the cover pose, then a one-frame command 0.25 m across
  ...
  makes it a wall pre-positioned from the chamber read, and Session 05 records which.
...
3. The grid, printed as a table into `../measurements.md` with the chosen committed shapes per
   weapon kind marked; the parry row beside it.
