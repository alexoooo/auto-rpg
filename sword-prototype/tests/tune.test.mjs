...
import {
  DEFAULT_LEAGUE, GENOME, INERT_ROWS, PLANNER_ROWS, census, defaultVector, designedBlockKey, diffVectors, evaluate, mutate,
  readStart, renderChampionsModule, tune,
...
/**
 * A checkpoint sweep rates a changing mind against three that do not change, and the three are
 * played once. The claim worth a test is not that fewer bouts run -- it is that the rows a kept
 * block hands back are the rows playing it would have written, in the positions they would have
 * had, so every column and the Elo walk read the same table either way.
 */
test("a_designed_contenders_bouts_are_kept_between_calls_and_replay_as_the_bouts_they_were", async () => {
  const pool = buildPool({ seed: SEED, random: 0 }).filter((build) => build.name === "default" || build.name === "fists");
  const shape = { league: ["golem-duelist"], pool, seed: SEED, bouts: 2, cap: 3, mirror: true, viable: false };
  const driver = { policy: "golem-driver" };
  const key = designedBlockKey({ name: "driver", contender: driver, ...shape });
  assert.equal(typeof key, "string");
  assert.equal(designedBlockKey({ name: "driver", contender: driver, ...shape }), key, "the key is its inputs and nothing else");
  assert.notEqual(designedBlockKey({ name: "driver", contender: driver, ...shape, seed: SEED + 1 }), key);
  assert.notEqual(designedBlockKey({ name: "driver", contender: driver, ...shape, pool: pool.slice(0, 1) }), key);
  assert.notEqual(designedBlockKey({ name: "driver", contender: driver, ...shape, mirror: false, viable: true }), key);
  assert.notEqual(designedBlockKey({ name: "fencer", contender: driver, ...shape }), key, "the rows carry the name");
  assert.equal(designedBlockKey({ name: "fit", contender: { pi: [0] }, ...shape }), null, "a mind with weights is not kept");
  assert.equal(designedBlockKey({ name: "driver", contender: driver, ...shape, record: ["driver"] }), null,
    "a call that records is a rollout and is not kept");

  const contenders = { uniform: { uniform: true }, driver };
  const run = { contenders, league: shape.league, pool, seed: SEED, bouts: 2, workers: 2, cap: 3 };
  const plain = await evaluate(run);
  const cache = new Map();
  const played = [];
  const first = await evaluate({ ...run, cache, onProgress: (p) => played.push(p.total) });
  const replayed = [];
  const second = await evaluate({ ...run, cache, onProgress: (p) => replayed.push(p.total) });
  assert.equal(cache.size, 2);
  assert.equal(played.at(-1), plain.rows.length);
  assert.deepEqual(replayed, [], "the second call played no bout");
  assert.deepEqual(first.rows, plain.rows);
  assert.deepEqual(second.rows, plain.rows);
  assert.deepEqual(second.results, plain.results);
  assert.deepEqual(second.rows.map((row) => row.index), plain.rows.map((_, i) => i));
});
